...
bookDetailModule.controller('BookDetailCtrl', function($scope, $http, $state, $stateParams) {
    $http.get('data/books' + $stateParams.bookId + '.json')
        .success(function(data){
          return  $scope.datas=data;
        });
        $scope.gridDetail={
            data:'datas',
            rowTemplate: '<div style="height: 100%"><div ng-style="{ \'cursor\': row.cursor }" ng-repeat="col in renderedColumns" ng-class="col.colIndex()" class="ngCell ">' +
...
var bookSearchModule = angular.module("BookSearchModule", []);
bookSearchModule.controller('BookSearchCtrl',function($scope,$http,$state,$stateParams){
  $http.get('data/books'+$stateParams.bookId+'.json');
